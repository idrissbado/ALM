#* @apiTitle ALM  API
#* @apiDescription RISKLOGICS ALM API 
...
#initializing script
########################################################################
#*return fitted spot yields on the specified dates using the market data:
#*@param req1
#*@param req2
#*@param NOW 
#*@param first
#*@param end 
#*@param typ
#*@post / fittedspotyields 
function(req1,req2,NOW,first,end,typ){
  portfolio <- read.csv(text=req1$postBody, 
                        header = TRUE, 
                        sep = ",", quote = "\"", dec = ".", fill = TRUE, comment.char = "")
  market <- read.csv(text=req2$postBody, 
                     header = TRUE, 
                     sep = ",", quote = "\"", dec = ".", fill = TRUE, comment.char = "")
  names(portfolio)<-c("id","account","account_name","volume","ir_binding","reprice_freq","spread","issue","maturity","repayment","payment_freq","yieldcurve")
  names(market)<-c("type","date","rate","comment")
  #format date values
  ...
  NOW<-as.Date("NOW",    format = "%m/%d/%Y")
  test.date<-seq(from= as.Date("first", format= "%m/%d/%Y"),
                 to= as.Date("end", format= "%m/%d/%Y"), by="1 year")
  get.yieldcurve.spot(market, test.date, type="typ", now=NOW, showplot=TRUE)
}
#* return cash-flow generation
#* @param req portfolio data 
#* @param no 
#* @serializer json
#* @post /cash-flowgeneration
function(req,no){
  portfolio<-read.csv(text=req$postBody,header = TRUE, 
                      sep = ",", quote = "\"", dec = ".", fill = TRUE, comment.char = "")
  NOW<-as.Date(as.character(no),    format = "%m/%d/%Y")
